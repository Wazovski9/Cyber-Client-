...
public final class GlyphPageFontRenderer {
	...
	public static GlyphPageFontRenderer createFromID(CharSequence id, int size, boolean bold, boolean italic,
		...
		try {
			...
			InputStream inputStream = MinecraftClient.getInstance().getResourceManager().getResource(identifier).get().getInputStream();
			font = Font.createFont(Font.TRUETYPE_FONT, inputStream).deriveFont(Font.PLAIN, size);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			if (bold) {
				InputStream inputStream = MinecraftClient.getInstance().getResourceManager().getResource(new Identifier(id.toString())).get().getInputStream();
				boldPage = new GlyphPage(
						Font.createFont(Font.TRUETYPE_FONT, inputStream).deriveFont(Font.BOLD, size),
						true, true);
			...
			if (italic) {
				...
				italicPage = new GlyphPage(
						Font.createFont(Font.TRUETYPE_FONT, inputStream).deriveFont(Font.ITALIC, size),
						true, true);
			...
			if (boldItalic) {
				...
				boldItalicPage = new GlyphPage(
						Font.createFont(Font.TRUETYPE_FONT, inputStream).deriveFont(Font.BOLD | Font.ITALIC, size),
						true, true);
	...
	public static GlyphPageFontRenderer createFromFont(Font font, int size, boolean bold, boolean italic, boolean boldItalic) {
		char[] chars = new char[256];
		for (int i = 0; i < chars.length; i++) {
			chars[i] = (char) i;
		}

		GlyphPage regularPage = new GlyphPage(font.deriveFont(Font.PLAIN, size), true, true);
		regularPage.generateGlyphPage(chars);
		regularPage.setupTexture();
