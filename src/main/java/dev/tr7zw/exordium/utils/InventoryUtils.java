...
import dev.tr7zw.exordium.mixin.ClientPlayerInteractionManagerAccessor;
	...
	public static int findRandomPot(String potion) {
		...
		for (int i = 0; i < 27; i++) {
			...
			if (itemStack.getItem() instanceof SplashPotionItem && (index != 36 || index != 37 || index != 38 || index != 39)) {
				List<StatusEffectInstance> effects = PotionUtil.getPotionEffects(itemStack);
				if (effects.stream().noneMatch(effect -> effect.getEffectType().getTranslationKey().equals(potion)))
					return -1;
	...
	public static int findPot(StatusEffect effect, int duration, int amplifier) {
		...
		for (int index = 9; index < 34; index++) {
			...
			if (stack.getItem() instanceof SplashPotionItem) {
				...
				if (effects.stream().anyMatch(e -> 
					e.getEffectType() == effect && 
					e.getDuration() == duration && 
					e.getAmplifier() == amplifier)) {
					return index;
				}
			}
		}
