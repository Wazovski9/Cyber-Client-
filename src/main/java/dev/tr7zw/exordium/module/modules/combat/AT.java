...
import dev.tr7zw.exordium.event.events.HudListener;
...
import dev.tr7zw.exordium.utils.*;
import dev.tr7zw.exordium.utils.rotation.Rotation;
...
public final class AT extends Module implements HudListener, MouseMoveListener {
	private final BooleanSetting trackerAim = new BooleanSetting(EncryptedString.of("Tracker Aim"), false);
	...
	private final NumberSetting radius = new NumberSetting(EncryptedString.of("Radius"), 0.1, 10, 5, 0.1);
	...
	public AT() {
		...
		addSettings(trackerAim, aimAt, stopAtTargetVertical, stopAtTargetHorizontal, radius, seeOnly, lookAtNearest, fov, pitchSpeed, yawSpeed, speedChange, randomization, yawAssist, pitchAssist, waitFor, lerp, posMode);
	}
	...
	public void onRenderHud(HudEvent event) {
		...
		if (trackerAim.getValue() && mc.player.getAttacking() instanceof PlayerEntity player && player.distanceTo(mc.player) < radius.getValue())
			target = player;
		...
		Vec3d targetPos = posMode.isMode(PosMode.Normal) ? target.getPos() : target.getLerpedPos(MinecraftClient.getInstance().getTickDelta());
		...
		if (lerp.isMode(LerpMode.EaseOut)) {
			yaw = (float) easeOutBackDegrees(mc.player.getYaw(), rotation.yaw(), yawStrength * MinecraftClient.getInstance().getLastFrameDuration());
			pitch = (float) easeOutBackDegrees(mc.player.getPitch(), rotation.pitch(), pitchStrength * MinecraftClient.getInstance().getLastFrameDuration());
		}
